...
public class JMainFrame extends JKBCTFrame {
  ...
  public String OrigKBDataFile="";
  private String SelectedVariablesDataFile="";
  private String oldIKBFile="";
  private String oldKBExpertFile="";//aquiiiiiiiiiiiii
  private String oldKBDataFile=""; //aquiiiiiiiii
...
//------------------------------------------------------------------------------
  private void DataSelection(double thres, JFIS fisaux, boolean automatic) {
     // válido para clasificación con una variable de salida
     try {
       if (this.DataFileNoSaved != null) {
...
//------------------------------------------------------------------------------
  void jMenuDataInduceRules_actionPerformed(boolean automatic, boolean cluster) {
    try {
      ...
      //   3) Está abierto el archivo de datos a partir del cual se incucirán las reglas
      ...
      if (this.kbct.GetNbOutputs()==0) {
        ...
        // Antes de seleccionar algoritmo, peguntar si se quiere selección de datos.
        ...
        // Seleccionar el algoritmo de inducción de reglas
        if (!end) {
          if (!automatic) {
...
//------------------------------------------------------------------------------
 public void jMenuExpertOpen_actionPerformed() { //aquiiiiiiiiiii
    ...
    JFileFilter filter2 = new JFileFilter(("XML").toLowerCase(), LocaleKBCT.GetString("FilterKBFile"));
      ...
      try {
        boolean warning= false;
        JKBCTFrame.KBExpertFile =  "DATOS.txt.kb.xml";
...
//------------------------------------------------------------------------------
// Sólo usado desde MainKBCT.java para crear archivos IKB de forma automática
...
//------------------------------------------------------------------------------
// Sólo usado por MainKBCT.java para crear archivos IKB de forma automática  
          ...
          try {
            if (this.jef.Temp_kbct!=null) {
              if (this.jef.Temp_kbct.GetKBCTFile()==null) {
            	...
            	JKBCTFrame.KBExpertFile= "DATOS.txt.kb.xml"; //cambiaaaaar
...
//------------------------------------------------------------------------------
// Sólo usado por MainKBCT para crear archivos IKB de forma automática
...
//------------------------------------------------------------------------------
  public void dispose() {
    ...
    super.dispose();
    if( JKBCTFrame.JMainFrames.size() == 0 )// Si es la única ventana abierta
