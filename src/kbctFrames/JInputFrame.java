...
/**
 * kbctFrames.JInputFrame generates a window with all information about one input.
 *
 *@author     Jose Maria Alonso Moral
 *@version    3.0 , 03/08/15
 */
public class JInputFrame extends JVariableFrame {
  static final long serialVersionUID=0;	
//------------------------------------------------------------------------------
  protected JInputFrame( JExpertFrame parent ) { super( parent ); }
//------------------------------------------------------------------------------
  public JInputFrame( JExpertFrame parent, JKBCT kbct, int input_number ) {
    super(parent);
    this.parent= parent;
    this.kbct = kbct;
    this.Number = input_number;
    this.Temp = this.kbct.GetInput(input_number+1);
    this.Semaphore = this.kbct.GetInputSemaphore(input_number);
  }
//------------------------------------------------------------------------------
  protected void TranslateComponents() {
    this.setTitle(LocaleKBCT.GetString("Input"));
    super.TranslateComponents();
    //System.out.println(this.jCBType.getSelectedIndex());
    //System.out.println(this.parent.getJExtDataFile().DataLength());
      ...
      try {
        switch (this.jCBType.getSelectedIndex()) {
          case 0: this.Temp.SetType("numerical");
                  ...
                  this.kbct.ReplaceInput(this.Temp.GetPtr(), (JKBCTInput)this.Temp);
        ...
        if (!t.toString().equals("java.lang.NullPointerException"))
            MessageKBCT.Error(this, LocaleKBCT.GetString("Error"), "Error in JInputFrame: jType_itemStateChanged: "+t);
      }
    }
  }
//------------------------------------------------------------------------------
    ...
    if( e.getStateChange() == ItemEvent.SELECTED ) {
      try {
        ...
        if (this.Temp.GetV().GetType().equals("logical")) {
            ...
            this.Temp.SetInputPhysicalRange(1, 2);
            this.Temp.SetInputInterestRange(1, 2);
            JInputFrame jif = new JInputFrame(super.parent, this.kbct, this.Number);
            jif.Show();
            this.dispose();
        } else {
          ...
          //System.out.println("input: "+this.Temp.GetPtr());
          this.kbct.ReplaceInput(this.Temp.GetPtr(), (JKBCTInput)this.Temp);
        }
      } catch (Throwable t) {
    	  t.printStackTrace();
        if (!t.toString().equals("java.lang.NullPointerException"))
          MessageKBCT.Error(this, LocaleKBCT.GetString("Error"), "Error in JInputFrame: jNOL_itemStateChanged: "+t);
      }
    }
  }
//------------------------------------------------------------------------------
  void jLL_itemStateChanged(ItemEvent e) {
      ...
      try {
          ...
          if (!t.toString().equals("java.lang.NullPointerException"))
              MessageKBCT.Error(this, LocaleKBCT.GetString("Error"), "Error in JInputFrame: jLL_itemStateChanged: "+t);
      }
    }
  }
//------------------------------------------------------------------------------
    ...
    if ( (JVariableFrame.AuthorizationButtonNewScale==JOptionPane.YES_OPTION) && (JVariableFrame.JButtonNewScalePressed) ) {
        try {
          ...
          if (Labels_Names != null) {
            ...
            MessageKBCT.Error(this, LocaleKBCT.GetString("Error"), "Error in JInputFrame: jButtonNewScale_actionPerformed: "+t);
        }
    }
  }
//------------------------------------------------------------------------------
    ...
    if ( (JVariableFrame.AuthorizationButtonChangeName==JOptionPane.YES_OPTION) && (JVariableFrame.JButtonChangeNamePressed) ) {
        try {
          ...
          if (Labels_Names != null) {
            ...
            MessageKBCT.Error(this, LocaleKBCT.GetString("Error"), "Error in JInputFrame: jButtonChangeName_actionPerformed: "+t);
        }
    }
  }
//------------------------------------------------------------------------------
  void jButtonSetNbLabels_actionPerformed() {
	  ...
	  if ( (JVariableFrame.AuthorizationButtonSetNbLabels==JOptionPane.YES_OPTION) && (JVariableFrame.JButtonSetNbLabelsPressed) ) {
        ...
        jButtonApply.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) {
          int NbLab= JInputFrame.this.jNbLabels.getValue();
          boolean warning= false;
          if (NbLab < 2) {
            MessageKBCT.Information(JInputFrame.this, LocaleKBCT.GetString("NbLabelsShouldBe"));
            warning= true;
          } 
          if (!warning) {
        	JInputFrame.this.UserNbLabels= NbLab;
        	JInputFrame.this.Temp.SetScaleName("user");
        	JInputFrame.this.Temp.SetLabelsNumber(NbLab);
        	JInputFrame.this.Temp.SetLabelsName();
        	JInputFrame.this.Temp.SetORLabelsName();
        	JInputFrame.this.Temp.SetLabelProperties();
            if (JInputFrame.this.Temp.GetV().GetType().equals("categorical")) {
            	JInputFrame.this.Temp.SetInputPhysicalRange(1, JInputFrame.this.Temp.GetLabelsNumber());
            	JInputFrame.this.Temp.SetInputInterestRange(1, JInputFrame.this.Temp.GetLabelsNumber());
              }
        	JInputFrame.this.kbct.ReplaceInput(JInputFrame.this.Temp.GetPtr(), (JKBCTInput)JInputFrame.this.Temp);
            jd.dispose();
          }
        } } );
        ...
        jButtonDefault.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) {
    	  JInputFrame.this.jNbLabels.setValue(JInputFrame.this.Temp.GetLabelsNumber());
        } } );
...
//------------------------------------------------------------------------------
  void jButtonAdvOpt_actionPerformed() {
    ...
    if ( (JVariableFrame.AuthorizationButtonAdvancedOptions==JOptionPane.YES_OPTION) && (JVariableFrame.JButtonAdvancedOptionsPressed) ) {
        try {
          JAdvOptFrame jaof = new JAdvOptFrame(this, this.kbct, this.Number, this.Temp, "Input");
          jaof.setLocation(JChildFrame.ChildPosition(this, this.getSize()));
          jaof.Show();
        } catch (Throwable t) {
            MessageKBCT.Error(this, LocaleKBCT.GetString("Error"), "Error in JInputFrame: jButtonAdvOpt_actionPerformed: "+t);
        }
    }
  }
//------------------------------------------------------------------------------
    ...
    if (warning) {
      ...
      try {
        ...
        jif.Show();
          ...
          MessageKBCT.Error(this, LocaleKBCT.GetString("Error"), "Error in JInputFrame: consistency(): "+t);
      }
      this.dispose();
    } else
       this.dispose();
