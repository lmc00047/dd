          ...
          if (MainKBCT.getConfig().GetPremiseRemoval()) {
        	 ...
        	 while (!loopend) {
        	   ...
        	   if (TM>=MM) {
                   ...
                   if (warning) {
            	       change=true;
                       // la segunda vez elimina redundancias aparecidas al forzar la eliminación de premisas
            	       NbRold= kbctaux.GetNbRules();
...
//------------------------------------------------------------------------------
  ...
  public JKBCT solveConflict ( JKBCT kbctexp, JKBCT kbct, int[] SelectedRules, File KBfileLinks, File FISfileLinks, JExtendedDataFile jedf, boolean expand, boolean commonPart, boolean split) {
    ...
    int NbExpR1=0;
    int NbExpR2=0;
    // expandir las reglas que producen el conflicto a su forma canónica
    if (expand) {
    ...
    ////////////
    // crear un fis sin reglas al que se añadira cada vez solo las reglas conflictivas
    JKBCT kbctaux= new JKBCT(kbct);
    //long ptr= kbctaux.GetPtr();
    ...
    if (expand) {
      ...
      // comparar las reglas expandidas para ver cuales producen conflicto (igual premisa pero diferente conclusión)
